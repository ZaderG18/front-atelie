...
const MOCK_STATS = {
  ...
  listaInsumosCriticos: [
    { id: 1, name: "Farinha de Trigo", stock_quantity: 2, min_stock_alert: 10, unit: "kg" },
    { id: 2, name: "Leite Condensado", stock_quantity: 5, min_stock_alert: 15, unit: "un" },
  ],
}
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Usando dados de demonstração:", error)
      } finally {
        setIsLoading(false)
      }
    }
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {/* Ticket Médio */}
            <Card className="relative overflow-hidden bg-gradient-to-br from-violet-500 to-violet-600 border-0 text-white">
              <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -translate-y-1/2 translate-x-1/2" />
              <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <Link href="/admin/pedidos/novo">
                  <Button
                    variant="outline"
                    className="w-full h-auto py-4 flex-col gap-2 bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-950/50 text-blue-700 dark:text-blue-400"
                  >
                ...
                <Link href="/admin/pedidos">
                  <Button
                    variant="outline"
                    className="w-full h-auto py-4 flex-col gap-2 bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800 hover:bg-amber-100 dark:hover:bg-amber-950/50 text-amber-700 dark:text-amber-400"
                  >
                ...
                <Link href="/admin/insumos">
                  <Button
                    variant="outline"
                    className="w-full h-auto py-4 flex-col gap-2 bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800 hover:bg-emerald-100 dark:hover:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400"
                  >
                ...
                <Link href="/admin/produtos">
                  <Button
                    variant="outline"
                    className="w-full h-auto py-4 flex-col gap-2 bg-violet-50 dark:bg-violet-950/30 border-violet-200 dark:border-violet-800 hover:bg-violet-100 dark:hover:bg-violet-950/50 text-violet-700 dark:text-violet-400"
                  >
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <Card className="lg:col-span-2 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button
                  ...
                  size="sm"
                  asChild
                  className="dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700 bg-transparent"
                >
                  ...
                  {stats.recentOrders && stats.recentOrders.length > 0 ? (
                    stats.recentOrders.map((pedido: any) => {
                      ...
                      return (
                        <div
                          key={pedido.id}
                          className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-200 border border-slate-100 dark:border-slate-700/50"
                        >
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center text-slate-600 dark:text-slate-300 font-semibold text-sm">
            ...
            <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                >
                  <Link href="/admin/insumos">
                    <Package className="mr-1 h-3 w-3" /> Ver
                  </Link>
                </Button>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-4">
                    {stats.listaInsumosCriticos.map((insumo: any) => {
                      ...
                      return (
                        <div
                          key={insumo.id}
                          className="space-y-2 p-3 bg-red-50 dark:bg-red-950/20 rounded-lg border border-red-100 dark:border-red-900/30"
                        >
                          ...
                          <Progress
                            value={porcentagem > 100 ? 100 : porcentagem}
                            className="h-2 bg-red-200 dark:bg-red-900/50"
                          />
                        </div>
                      )
                    })}
